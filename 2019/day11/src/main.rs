...
fn panels_painted(program: Vec<i128>) -> usize {
    let panels: HashMap<Point, Color> = HashMap::new();
    let robot = paint_hull(program, panels);
    robot.painted.len()
}
...
fn paint_hull(program: Vec<i128>, panels: HashMap<Point, Color>) -> Robot {
