defmodule Day4 do
  ...
  def sleepy_minute_product(lines) do
    {guard_id, minute_slept, _} =
      lines
      |> guard_minutes()
      |> Enum.map(fn {guard_id, minutes_slept} ->
        {most_slept_minute, times} =
          minutes_slept
          |> Enum.group_by(& &1)
          |> Enum.max_by(fn {_k, v} -> Enum.count(v) end)

        {guard_id, most_slept_minute, times |> Enum.count()}
      end)
      |> Enum.max_by(fn {_, _, slept_minutes} -> slept_minutes end)

    guard_id * minute_slept
  end

  defp guard_minutes(lines) do
    lines
    |> Enum.sort()
  ...
  ["--test"] ->
    ...
    defmodule Day4Test do
      ...
      test "sleepy_minute_product" do
        ...
        assert sleepy_minute_product(io |> IO.stream(:line) |> Stream.map(&String.trim/1)) == 4455
      end
    end
